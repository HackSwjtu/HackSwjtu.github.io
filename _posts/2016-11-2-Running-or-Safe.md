...
*运动世界校园* 拥有一套很有意思的跑步规则，在选择目标距离后，在地图上回“随机”给出多个待检测点，而在这些点中，你必须要经过几个，路线随意，这样才完成了跑步评测的第一步。而在跑步结束后，软件还会计算你的跑步平均速度，如果偏差太大（过大或过慢）也不会记录成为有效成绩。
...
这是一个及不负责任的认证提交数据的方式。试想，如果用户非正常方式进行提交数据，因为不经过分发 token 进行在线认证，**很容易就能造出虚假数据出来**。
...
在斥责开发 app 所在公司的同时，也提醒广大用户多加注意个人隐私的保护，提高个人信息的安全意识，在有法却无严厉监管的环境下我们只有提高个人。（这实属无奈之举。）

## 写在最后

这篇文章是我们 [HackSwjtu]() 的第一篇公示文，以此希望 *运动世界校* 的公司 **杭州万航信息科技有限公司** 早日对数据问题进行修复更新，也希望我交大的信息学院在关注学科排名的同时，提升学院技术。
